...
export default function App() {
  ...
  function addMonitor(event: FormEvent<HTMLFormElement>) {
    ...
    setMonitors((current) => [{ id: Date.now(), name: String(data.get("name")), source: String(data.get("source")), expected: String(data.get("expected")), status: "Waiting", received: "Not received yet", recipients: ["You"] }, ...current]);
    setDialogOpen(false); setToast("Monitor added to this demo"); window.setTimeout(() => setToast(""), 3000);
  }
